...
# Run the CLI lookup (reads .env for START_LOC, ARTIST_URL, and optional GOOGLE_MAPS_API_KEY)
...
Required `.env` values: `START_LOC` (used by `finder.py`'s CLI path and as the default prefill for `/api/config`) and `ARTIST_URL` (CLI-only). `GOOGLE_MAPS_API_KEY` is optional; quota, auth, network, or missing-key failures fall back to rate-limited/cached Nominatim geocoding plus an explicitly marked great-circle road-distance estimate. The web app never leaks the Maps API key to the client — see `/api/config` in `app.py`.
...
- `static/app.js` owns all frontend state: search/resolve flow, Leaflet/OpenStreetMap rendering (`initMap`, markers, popovers), list/map view toggle for mobile, sort and filter controls (distance, date range, sold-out), and the saved-artists chip list. There's no bundler — it's a single script loaded directly by `index.html`.
